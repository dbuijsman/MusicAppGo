...
func (handler *MusicHandler) AddNewArtist(artist, prefix, linkSpotify string) (types.Artist, error) {
	if artist == "" {
		handler.Logger.Printf("Can't add artist without a name: %v, %v\n", artist, prefix)
		return types.Artist{}, dberror.GetDBError("Missing artist", dberror.InvalidInput)
	}
	...
	if err != nil {
		if err.(dberror.DBError).ErrorCode == dberror.DuplicateEntry {
			...
			return types.Artist{}, dberror.GetDBError("This artist is already in the database", dberror.DuplicateEntry)
		}
		failedNewArtist.Inc()
		handler.Logger.Printf("[ERROR] Failed to add %v, %v due to: %s\n", artist, prefix, err)
		return types.Artist{}, dberror.ErrorToUnknownDBError(err)
	}
	go func(handler *MusicHandler, newArtist types.Artist) {
		msg, err := convert.ToJSONBytes(newArtist)
		if err != nil {
...
func (handler *MusicHandler) AddSong(song string, artists ...string) (types.Song, error) {
	if song == "" {
		handler.Logger.Printf("Can't add song without a name: %v - %v\n", artists, song)
		return types.Song{}, dberror.GetDBError("Missing song", dberror.InvalidInput)
	}
	if len(artists) == 0 {
		handler.Logger.Printf("Can't add song without artists: %v - %v\n", artists, song)
		return types.Song{}, dberror.GetDBError("Missing artists", dberror.InvalidInput)
	}
	...
	contributingArtists := make([]types.Artist, 0, len(artists))
	...
	go func(handler *MusicHandler, newSong types.Song) {
		msg, err := convert.ToJSONBytes(newSong)
		if err != nil {
